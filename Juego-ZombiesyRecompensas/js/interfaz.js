...
function inicialitzarTauler(medidaTauler) {
    ...
    taulerHTML(); //Incialitzem el tauler en HTML
    dibuixarTauler(); // Inicialitzem el en consola
    mostrarEstat(); // Mostrar i actualitzar l'estat
}

// Actualitzar el tauler HTML per poguer dibuixar-ho al nostre HTML
function taulerHTML() {
    const tabla = document.getElementById('tabla');
    tabla.innerHTML = ''; // Limpiar taula bans de dibuixar

    for (let i = 0; i < tauler.length; i++) {
        let fila = document.createElement('tr'); // Crear fila per la taula

        for (let j = 0; j < tauler[i].length; j++) {
            let celda = document.createElement('td'); // Crear cel.la per la taula

            if (tauler[i][j] === 1) {
                celda.textContent = '😀'; // Emoji del jugador
            } else if (tauler[i][j] === 4) {
                celda.textContent = '✔️'; // Casella visitada
            } else {
                celda.textContent = '⬜'; // Espaci buit
            }
            fila.appendChild(celda); // Afegir la cel.la a la fila
        }
        tabla.appendChild(fila); // Afegir fila a la taula
    }
}
...
function moureJugador(direccio) {
    ...
    taulerHTML(); // Actualizar tauler HTML
    mostrarEstat();
