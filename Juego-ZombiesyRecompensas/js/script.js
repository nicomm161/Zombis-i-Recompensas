...
function taulerHTML() {
    ...
    tabla.innerHTML = ''; // Limpiar tabla abans de dibuixar
    for (let i = 0; i < tauler.length; i++) {
        ...
        for (let j = 0; j < tauler[i].length; j++) {
            ...
            if (tauler[i][j] === 1) {
                ...
                celda.textContent = '✔️'; // Marqum com casella visitada
            } else {
                celda.textContent = '⬜'; // Emoji de espai buit
            }
            fila.appendChild(celda); // Afegir la cel.la a la fila
        }
        tabla.appendChild(fila); // Afegir fila al tauler
    }
}
